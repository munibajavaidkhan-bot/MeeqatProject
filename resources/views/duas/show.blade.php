    ...
    <div class="card p-8 md:p-12 relative overflow-hidden">
        ...
        <div class="flex justify-between items-start mb-8 relative z-10">
            ...
            <form action="{{ route('duas.bookmark', $dua->id) }}" method="POST">
                @csrf                            <button type="submit" class="p-2 rounded-full {{ $isBookmarked ? 'bg-secondary-500 text-white' : 'bg-dark-100 text-muted' }} hover:scale-110 transition" aria-label="Toggle bookmark">
        ...
        <h1 class="text-3xl font-black text-heading mb-8 relative z-10">{{ $dua->title_en }}</h1>
        
        <div class="bg-surface p-6 rounded-2xl border border-border mb-8 relative z-10">
            <p class="arabic text-3xl md:text-5xl text-secondary-600 leading-relaxed text-right mb-6">{{ $dua->arabic_text }}</p>
        </div>
        ...
        <div class="space-y-6 relative z-10">
            ...
            @if($dua->translation_ur)
                <div>
                    <h3 class="text-secondary-600 font-bold mb-2">اردو ترجمہ</h3>
